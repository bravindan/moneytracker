...
const ReportsScreen = ({ navigation, route }) => {
  ...
  const spent = spendings.reduce(
    (acc, curr) => acc + (curr.totalSpending || curr.amount || 0),
    0,
  );
  const totalTxnCosts = spendings.reduce(
    (acc, curr) => acc + (curr.transactionCosts || 0),
    0,
  );
  ...
  const savingsInvestments =
    (summary.savingsAmount || 0) + (summary.investmentAmount || 0);
  const savingsRate =
    summary.income > 0 ? (savingsInvestments / summary.income) * 100 : 0;

  // Categorize
  const categoryTotals = {};
  spendings.forEach((s) => {
    categoryTotals[s.category] =
      (categoryTotals[s.category] || 0) + (s.totalSpending || s.amount || 0);
  });
  const highestCategoryObj = Object.keys(categoryTotals).sort(
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ...
            <Text style={{ fontWeight: "bold" }}>
              {highestCategoryObj || "None yet"}
            </Text>{" "}
            ({fmt(categoryTotals[highestCategoryObj] || 0)})
          </Text>
        </View>
        ...
        >
          ...
          >
            <Text
              style={{ color: theme.colors.textSecondary, fontWeight: "bold" }}
            >
              Cash Available
            </Text>
            <Text
              style={{
                color: totalAllocated - spent >= 0 ? "#10b981" : "#ef4444",
                fontWeight: "bold",
              }}
            >
        ...
        >
          ...
          {investments.length > 0 ? (
            investments.map((inv, idx) => (
              <View key={idx} style={[styles.row, { marginBottom: 8 }]}>
                <View>
                  ...
                  >
                    Cost: {fmt(inv.transactionCosts || 0)}
                  </Text>
                </View>
                <Text
                  style={{
                ...
                >
                  {fmt(
                    inv.totalInvestment ||
                      inv.amount + (inv.transactionCosts || 0),
                  )}
                </Text>
              </View>
            ))
            ...
            <Text style={{ color: theme.colors.textSecondary }}>
              No dynamic investments mapped this month.
            </Text>
          )}
        </View>
      </ScrollView>
